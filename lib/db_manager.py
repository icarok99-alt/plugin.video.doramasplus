...
ADDON_ID = 'plugin.video.doramasplus'
ADDON = xbmcaddon.Addon(ADDON_ID)
ADDON_DATA = xbmcvfs.translatePath('special://profile/addon_data/{}/'.format(ADDON_ID))
DATABASE_PATH = os.path.join(ADDON_DATA, 'doramasplus.db')


def notify(message, time_ms=4000):
    xbmc.executebuiltin('Notification(DoramasPlus, {}, {}, {})'.format(
        message, time_ms, ADDON.getAddonInfo('icon')
    ))


class DoramasDatabaseManager:
    ...
    def delete_database(self, confirm=True):
        ...
        if confirm:
            ok = xbmcgui.Dialog().yesno('DoramasPlus', 'Deseja excluir o banco de dados?')
            if not ok:
                return False
...
if __name__ == '__main__':
    DoramasDatabaseManager().delete_database(confirm=True)